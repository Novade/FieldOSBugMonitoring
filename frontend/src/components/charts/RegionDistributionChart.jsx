...
import { REGION_ALL } from '../../utils/fieldUtils';
import { TOOLTIP_DEFAULTS } from '../../constants/chartDefaults';
import { makeDonutValueLabelsPlugin } from '../../utils/chartPlugins';

ChartJS.register(ArcElement, Tooltip, Legend);

const COLORS = ['#3b6cb7', '#2e7d5e', '#e67e22', '#6d28d9', '#0891b2'];

const donutValueLabels = makeDonutValueLabelsPlugin('regionDonutValueLabels');

export function RegionDistributionChart({ issues, onDrillTo }) {
  const { labels, datasets } = useMemo(() => {
    const counts = {};
    issues.forEach((b) => {
      b.reg?.forEach((r) => {
        if (REGION_ALL.includes(r)) counts[r] = (counts[r] || 0) + 1;
      });
    });
  ...
  if (labels.length === 0) {
    return (
      <div className="flex items-center justify-center h-full min-h-[160px]">
        <p className="text-[13px] text-[#8896b0]">No region data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-center h-full">
      ...
      <p className="text-[10px] text-[#a0aab8] mt-2 italic">
        * Issues affecting multiple regions are counted on each region.
      </p>
    </div>
  );
}
